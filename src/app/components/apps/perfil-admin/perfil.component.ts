...
export class PerfilComponent implements OnInit {
  ...
  public inscripcionCelulas: InscripcionCelulas[] = [];  public jovenes: any[] = [];
...
else if(this.userRole === 'ADMIN'){  this.http.get<Joven[]>(this.jovenApiUrl, { headers }).subscribe(
  (data) => {
const jovenEncontrado = data.find(joven => joven.correo === this.joven);
if (jovenEncontrado) {
  this.jovenesid = jovenEncontrado.id;
  ...
  retirarDelReto(retoId: number) {
    Swal.fire({
      title: '¿Estás seguro?',
      text: '¿Quieres retirar al joven de este reto?',
      icon: 'warning',
...
deleteJovenCelulas(celulaId: number) {
  ...
  Swal.fire({
    title: '¿Estás seguro?',
    text: 'Si la fecha de inscripción ya finalizo no podras inscribirte de nuevo',
    icon: 'warning',
    showCancelButton: true,
    confirmButtonText: 'Sí, retirar',
    cancelButtonText: 'Cancelar',
